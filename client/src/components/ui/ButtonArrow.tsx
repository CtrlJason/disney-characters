...
const ButtonArrow = ({ parentFunction, direccion }: ButtonArrowProps) => {
    return (
        <button
            onClick={parentFunction}
            className={`${direccion === "left" ? "left-4 md:left-10" : "right-4 md:right-10"} absolute top-1/2 -translate-y-1/2 text-4xl p-4 rounded-full cursor-pointer z-10 transition-transform duration-150 hover:scale-110 active:bg-gray-700 active:scale-105 bg-gray-800 shadow-lg`}
        >
            {direccion === "left" ? (

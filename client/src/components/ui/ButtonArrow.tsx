...
const ButtonArrow = ({ handleDirection, direccion }: ButtonArrowProps) => {
    return (
        <button
            ...
            className={`${direccion === "left" ? "left-0 md:left-10" : "right-0 md:right-10"} absolute top-1/2 -translate-y-1/2 text-4xl p-4 rounded-full cursor-pointer z-10 transition-transform duration-150 hover:scale-110 active:bg-gray-200 active:scale-105 bg-white`}
        >
            {direccion === "left" ? (
